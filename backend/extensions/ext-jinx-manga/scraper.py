...
class JinxMangaScraper(BaseScraper):
    name = "Jinx Manga"
    base_url = "https://jinxmanga.com"
    language = "en"
    nsfw = False
    version = "1.0.0"
    ...
    async def search(self, query: str) -> List[Manhwa]:
        ...
        for url in candidate_urls:
            try:
                ...
                if soup.select("div.bs, article.bs, div.utao, div.bsx, .post-title"):
                    break
            except Exception:
                continue
        ...
        for card in soup.select("div.bs, article.bs, div.utao, div.bsx"):
            ...
            cover = ""
            if cover_el:
                cover = cover_el.get("src", "") or cover_el.get("data-src", "") or cover_el.get("data-lazy-src", "")
        ...
        for row in soup.select(".post-title a, h3 a"):
    ...
    async def get_detail(self, manhwa_url: str) -> Manhwa:
        ...
        desc_el = soup.select_one("div.entry-content p, .summary__content, .synp p")
