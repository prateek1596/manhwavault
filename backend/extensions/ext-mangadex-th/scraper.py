...
class MangaDexThaiScraper(BaseScraper):
    name = "MangaDex (Thai)"
    ...
    async def search(self, query: str) -> List[Manhwa]:
        data = await self._get(
            ...
            params={
                ...
                "limit": 20,
                "includes[]": ["cover_art"],
                "availableTranslatedLanguage[]": ["th"],
            },
        )
    ...
    async def get_chapters(self, manhwa_url: str) -> List[Chapter]:
        ...
        async def fetch_chapters(translated_lang: Optional[str]) -> List[Chapter]:
            ...
            while True:
                ...
                for item in rows:
                    ...
                    chapters.append(
                        Chapter(
                            id=_make_id(f"mdc-th:{chapter_id}"),
        ...
        chapters = await fetch_chapters("th")
        if chapters:
            return chapters
