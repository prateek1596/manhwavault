...
class MangaDexIndonesianScraper(BaseScraper):
    name = "MangaDex (Indonesian)"
    ...
    async def get_chapters(self, manhwa_url: str) -> List[Chapter]:
        ...
        async def fetch_chapters(translated_lang: Optional[str]) -> List[Chapter]:
            ...
            while True:
                ...
                for item in rows:
                    ...
                    chapters.append(
                        Chapter(
                            id=_make_id(f"mdidc:{chapter_id}"),
        ...
        chapters = await fetch_chapters("id")
        if chapters:
            return chapters
