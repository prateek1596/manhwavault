...
class MangaDexPortugueseScraper(BaseScraper):
    name = "MangaDex (Portuguese)"
    ...
    async def get_chapters(self, manhwa_url: str) -> List[Chapter]:
        ...
        async def fetch_chapters(translated_lang: Optional[str]) -> List[Chapter]:
            ...
            while True:
                ...
                for item in rows:
                    ...
                    chapters.append(
                        Chapter(
                            id=_make_id(f"mdptc:{chapter_id}"),
        ...
        chapters = await fetch_chapters("pt-br")
        if chapters:
            return chapters

        chapters = await fetch_chapters("pt")
        if chapters:
            return chapters
