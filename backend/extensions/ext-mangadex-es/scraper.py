...
class MangaDexSpanishScraper(BaseScraper):
    name = "MangaDex (Spanish)"
    ...
    async def get_chapters(self, manhwa_url: str) -> List[Chapter]:
        ...
        async def fetch_chapters(translated_lang: Optional[str]) -> List[Chapter]:
            ...
            while True:
                ...
                for item in rows:
                    ...
                    chapters.append(
                        Chapter(
                            id=_make_id(f"mdesc:{chapter_id}"),
        ...
        chapters = await fetch_chapters("es")
        if chapters:
            return chapters

        chapters = await fetch_chapters("es-la")
        if chapters:
            return chapters
